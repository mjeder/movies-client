...
class MovieCreate extends Component {
  constructor (props) {
    super(props)
    // Add a movie object to state, with an initially empty
    // title and director
    this.state = {
      movie: {
        title: '',
      ...
      // include an id for the created movie. We will redirect to
      // this id, once our movie has been created
      createdMovieId: null
    }
  }
  handleChange = event => {
    // we will try to access the event after the handleChange function has finished
    // to make sure the event properties aren't null, we need to persist them
    event.persist()
    this.setState(state => {
  ...
  handleSubmit = event => {
    ...
    // destructure our props and state
  ...
  render () {
    ...
    // once our movie has been created and we stored its id
    if (createdMovieId) {
